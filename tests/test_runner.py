...
# ── Colours (Windows 10+ supports ANSI in most terminals) ────────────────────
_GREEN  = "\033[92m"
_RED    = "\033[91m"
_YELLOW = "\033[93m"
_CYAN   = "\033[96m"
...
def _run_test(entry: dict) -> dict:
    """Run a single registered test and return its result dict."""
    fn = entry["fn"]
...
def run_all(phase_filter: Optional[str] = None) -> List[dict]:
    """Run all registered tests (optionally filtered by phase name)."""
    results: List[dict] = []
    tests_to_run = [e for e in _REGISTRY
    ...
    print(f"{_BOLD}  ARIA Test Suite -- {total} tests{f' (phase: {phase_filter})' if phase_filter else ''}{_RESET}")
    ...
    print(f"\n{_BOLD}{_CYAN}{sep}{_RESET}")
    print(f"{_BOLD}  Results: {_GREEN}{n_pass} passed{_RESET}, {_RED}{n_fail} failed{_RESET}, {_YELLOW}{n_skip} skipped{_RESET}  /  {total} total{_RESET}")
    print(f"{_BOLD}{_CYAN}{sep}{_RESET}\n")
...
def _write_report(results: List[dict]) -> None:
    """Write a human-readable test report to tests/results/."""
    try:
        ...
        for r in results:
            status_sym = "[PASS]" if r["status"] == PASSED else ("[SKIP]" if r["status"] == SKIPPED else "[FAIL]")
            lines.append(
                f"[{r['phase']:12}] {status_sym}  {r['description']}"
                + (f"\n             Error: {r['error']}" if r["error"] and r["status"] == FAILED else "")
            )
...
def _write_report(results: List[dict]) -> None:
    ...
    try:
        ...
        for r in results:
            status_sym = "✓" if r["status"] == PASSED else ("⊘" if r["status"] == SKIPPED else "✗")
            lines.append(
                f"[{r['phase']:12}] {status_sym} {r['status']}  {r['description']}"
                + (f"\n             Error: {r['error']}" if r["error"] and r["status"] == FAILED else "")
            )
